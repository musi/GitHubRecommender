...
/****************************************************************************************************/
public class SVDItemBasedRecommenderImputation  
/****************************************************************************************************/
{
	...
	int upperDim 	= 60;		// Total dimensions to be tested
...
/****************************************************************************************************/
    ...
    public SVDItemBasedRecommenderImputation (boolean foldFlag, int X, int dataset) 
    {
    		
    		myPath = "C:/Users/Administrator/workspace/MusiRecommender/DataSets/SML_ML/SVD/";
	        ...
	        //Initialize results, Mean and SD	    	
	    	 array_MAE  	 	= new double[5][11][31];  //IBCF: [10]--> k=5;k<50;K+=5
	    	 array_MAEPerUser	= new double[5][11][31]; 
	    	 array_NMAE		 	= new double[5][11][31];  //IBCF: [10]--> k=10;k<100;K+=10
	    	 array_NMAEPerUser	= new double[5][11][31];
	    	 array_RMSE 	 	= new double[5][11][31];
	    	 array_RMSEPerUser 	= new double[5][11][31];
	         array_Coverage  	= new double[5][11][31];
	         array_ROC 		 	= new double[5][11][31];
	         array_BuildTime 	= new double[5][11][31];
	         
	         array_Precision 	= new double[8][5][11][31]; //[topN][fold][neigh][dim]
	         array_Recall 	 	= new double[8][5][11][31];
	         array_F1 		 	= new double[8][5][11][31];
	         	         
	         //So we have to print this grid result for each scheme,
	         //Print in the form of "mean + sd &" 
	         gridResults_Mean_MAE 			= new double[11][31];	// neigh, dim, see IBCF, and UBCF diff above	        
	         gridResults_Mean_NMAE			= new double[11][31];	         
	         gridResults_Mean_RMSE			= new double[11][31];
	         gridResults_Mean_MAEPerUser	= new double[11][31];
	         gridResults_Mean_RMSEPerUser	= new double[11][31];
	         gridResults_Mean_NMAEPerUser	= new double[11][31];
	         gridResults_Mean_ROC			= new double[11][31];
	         
	         gridResults_Mean_Precision		= new double[8][11][31];  // [toppN][neigh][dim]
	         gridResults_Mean_Recall		= new double[8][11][31];
	         gridResults_Mean_F1			= new double[8][11][31];       
	         	         
	         gridResults_Sd_MAE			= new double[11][31];	         
	         gridResults_Sd_NMAE		= new double[11][31];	         
	         gridResults_Sd_RMSE		= new double[11][31];
	         gridResults_Sd_NMAEPerUser	= new double[11][31];
	         gridResults_Sd_MAEPerUser	= new double[11][31];
	         gridResults_Sd_RMSEPerUser = new double[11][31];
	         gridResults_Sd_ROC			= new double[11][31];
	         
	         gridResults_Sd_Precision	= new double[8][11][31];
	         gridResults_Sd_Recall		= new double[8][11][31];
	         gridResults_Sd_F1			= new double[8][11][31];
    ...
    {    	
    	if(myChoice.equalsIgnoreCase("IBCF")){
    	   return (recommendItemBased(userId, movieId, neighbours));
    		...
    		myFilter_UBCF = new FilterAndWeight(myTrainingMMh,1); 		 //FilterAndWeight, Random object, For CF
    		 
    		return(myFilter_UBCF.recommendS(userId, movieId, neighbours, 1));
         			...
         			*/
         			...
         			//all
       				...
       				//ML	
       					...
       					if(activeUser<6040 && targetMovie < 3383)
       						activeUserRatingOnSimItem = Prediction_Matrix.get(activeUser-1, mid-1);
       				}
            ...
            {            
            	...
            	//if(activeItem<1682 && myItem<1682)
            	{
            		...
            		//ML (it is also like FT5)
            			...
            			if(activeUser<6040 && activeItem < 3383)
            			{
            				rating1 = right.get(i, activeItem-1);                
    		                rating2 = right.get(i, myItem-1);
            			}
...
/****************************************************************************************************/
  ...
  /*          
        ...
        //	     currentWeight  = findUserSimilarity_SVD(activeUser, uid, targetMovie); //active item, item we want to find sim with
        	...
        	//All movies, as in case of SVD, a user have rated all movies (100% coverage)
            IntArrayList users = myMainMMh.getListOfUsers();
            totalUsersWhoSawTargetMovie = users.size();	   
        	activeUserAverage = myTrainingMMh.getAverageRatingForUser(activeUser);   //active user average
...
/****************************************************************************************************/
    ...
    public static void main(String[] args) 
    {
    	 ...
    	 int      X			 = 80;				// 100 for sml, 80 for ft (infact x=0.8 in both cases)
    	 int      dataset	 = 2;				// 0=sml, 1=ft, 2=ml 
    	  
    	SVDItemBasedRecommenderImputation svdRec = new SVDItemBasedRecommenderImputation(foldingFlg,		// FiveFold?													    			 
													    			 X,					// training set ratio
													    			 dataset    		// dataset 
    																);

    	//We have to change it for FT only
    	svdRec.FTDataWithMinRat = 1;
 ...
 */
    ...
    {   
    	...
    	if(FTFlag==false) {
    		...
    		{
    			 myTotalFolds 	= 5;			//only one fold or 5 folds
	    		 myClasses 		= 5;
	    		 TopNThreshold 	= 4;
	    		 end = 1;
    		}
    	...
    	for(int m=2;m<=2;m++)  //loop to control the minUsers and Mov dataset in FT
    	{
    		if(m==0)
    			FTDataWithMinRat = 1;
    		else if(m==1)
  ...
  /*  	 myChoice = "UBIBCF";
    	 ub_Opt_neigh = 15;					
	     ib_Opt_neigh = 10;
	     ub_Opt_K	  = 44;			//should change them to optimal				
	     ib_Opt_K 	  = 48;	        
...
/****************************************************************************************************/
  ...
  {      	
		...
		for(int tempVar = 1;tempVar<=10;tempVar++)
    	{
    		//double myLimit    = tempVar * 1.0;
    		double myLimit    = 10.0;
    		double limit_user = tempVar * 1.0;
      ...
      //-----------------------------
		...
		if(dataset==0){
			if(sparse ==false){
					...
					//FEATUREPLAY
					path ="C:/Users/Musi/workspace/MusiRecommender/DataSets/SML_ML/SVD/FeaturesPlay/80/";
				}
			else
				path = "C:/Users/Musi/workspace/MusiRecommender/DataSets/SML_ML/SVD/" + xFactor + "/";
    	...
    	if(myChoice.equalsIgnoreCase("UBIBCF")){    //For Hybrid Rec, neighbours and Dim are constant
   ...
   for (int ubCoff=0;ubCoff<=10;ubCoff++)
   {
    for(int ibCoff=0;ibCoff<=10;ibCoff++)
    {
      ...
      {  
    	...
    	{    
   			if(dataset==0){   				
	    		...
	    		// svdFile = path  + modelNormalization +"/" + modelName +"_More" + myLimit+ "_" + trainingOrFullSet + "_"+ modelNormalization  +".dat";
	    		 svdFile = path  + modelNormalization +"/" + modelName + myLimit +limit_user+ "_" + trainingOrFullSet + "_"+ modelNormalization  +".dat";
		    		...
		    		/*	 train  = path + "sml_trainSetStoredFold" +(fold) + ".dat";
				    	 ...
				    	 */
		    			 ...
		    			 train  = path + "sml_clusteringTrainingTrainSetStoredTF.dat";
		    			 test   = path + "sml_clusteringTrainingValidationSetStoredTF.dat"; 	   
		    			 mainObj = path + "sml_storedFeaturesRatingsTF.dat";
   			...
   			*/
	   		...
	   		/*	  train = path + "ft_clusteringTrainSetStored"+ FTInfo + "TF" + FTDataWithMinRat +".dat";
		          ...
		          mainObj = path + "ft_storedFeaturesRatingsTF.dat";*/
   			...
   			//ML
	    		 ...
	    		 path ="C:/Users/Musi/workspace/MusiRecommender/DataSets/ML_ML/SVD/FiveFoldData/"+ xFactor + "/";
	    	...
	    	{ 		
	    	 ...
	    	 // Loop over Dimensions 
		        ...
		        {	
			      ...
			      /*  System.out.println("Fold: "+ fold + ",k: "+ i + 
			        					", Neighbours:" + n + ", MAE= "+ MAE);
			        					*/
	  ...
	  //--------------------------------------------------
	   ...
	   //I think, first comparison is ok with top-20, then you can further draw a 
	    ...
	    if(!(myChoice.equalsIgnoreCase("UBIBCF"))) //do not write into file for hybrid approach
	    {
		   	openFiles(path);	    
		    writeDataIntoFiles();
		    closeFiles(); 
    ...
    /************************************************************************************************/
    		    ...
    		    public void testWithMemHelper(MemHelper testmh, int neighbours)     
    		    {
    		        ...
    		        //System.out.println("sd mae="+ SDInMAE);
    		         //-------------------------------------------------
    		         //Calculate top-N    		            
    ...
    { 				 
    	  ...
    	  { 
    		 for(int neigh=0; neigh< neighbourLimit ; neigh++)
    	     {
			  ...
			  for(int top =0;top<topNLimit;top++)
			  {
    	    	...
    	    	for(int dim=0; dim<=31;dim++) 	 //at 16 write "\n"
    	    	 {    	    	    			
    	    		  if(dim<=30)				//write values
    	    		   {		    				 
		    				 //start writing values
		    				 double result =0;		    			    				 
		    				 ...
		    				 else if(i==1)		    					 
		    				 {
			    				 if(j==0)	  {  	result = gridResults_Sd_MAE[neigh][dim];
			    				 					//System.out.println("sdm="+  gridResults_Sd_MAE [0][0] );
			    				 			}
     ...
     //------------------------------------------------    		    	 
	    	 ...
	    	 public void openFiles(String path)
	    	 {
	    		if(FTFlag==false){
	    			if(sparse ==false)
	    				path +="Results/" + trainingOrFullSet + "/" + modelNormalization + "/"  + myChoice +  "/" ; //+ "Previous/";
	    			else
	    				path +="Results/" + trainingOrFullSet + "/" + modelNormalization + "/"  + myChoice +  "/Sparse/";
	    		}
	    		else
	    			path += modelNormalization + "/" + FTDataWithMinRat + "/" + "Results" +
	    					"" + "/" + myChoice +  "/" ;   //+ "All/";
	    		 
	    		 try{
	    			 myWriter[0][0] = new FileWriter(path +trainingOrFullSet+ "_"+ "MeanMAE_New"+ myChoice+ modelNormalization+ ".csv", true); //true, append in exisiting file
	    			 ...
	    			 myWriter[1][0] = new FileWriter(path +trainingOrFullSet+ "_"+"SDMAE_New"+ myChoice+ modelNormalization+".csv", true); //true, append in exisiting file
