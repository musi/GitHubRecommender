...
/******************************************************************************************************/	
	@Override
	//called from Abstract recommender
	public double recommend(int uid, int mid, String date) 
	{
		...
		for (int i=lowerRating; i<=upperRating; i++) 
		{
			...
			//if we're following the paper, keep track of the max probability
			else  
			{
				if (current >= max) 
				{
...
/******************************************************************************************************/	
	...
	public static void main (String[] args)
	{
        ...
        for (int i=1; i<6; i++)
        {
   		 	String memHelperTrain = "C:\\Users\\Musi\\workspace\\MusiRecommender\\DataSets\\SML_ML\\PD\\FiveFoldData\\sml_trainSetStoredFold" + (i+1) + ".dat";
   		    String memHelperTest  = "C:\\Users\\Musi\\workspace\\MusiRecommender\\DataSets\\SML_ML\\PD\\FiveFoldData\\sml_testSetStoredFold" + (i+1) + ".dat";
   		    String simFiles       = myPath + "\\StoredRCSim\\SimFold" + (i+1) + ".dat";
   		    	
        	rec = new PDRecommender(simFiles,											// Sim-keeper 
        							memHelperTrain, 									// MemReader
            						false, 
            						true); 		 
            
        	h = new MemHelper(memHelperTest);
            
            System.out.println("Ready to start recommendations.");
            time.resetTimer();
            time.start();
            rmse = rec.testWithMemHelper(h, 25);						//call rec method
            time.stop();
