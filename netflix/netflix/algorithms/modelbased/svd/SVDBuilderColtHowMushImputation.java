 ...
 * @author sowellb
 */
class SVDBuilderColtHowMushImputation 
{
	...
	public SVDBuilderColtHowMushImputation(int myClasses, boolean folding, boolean FTFlag,  String trainingOrFullSet)
	{
...
/****************************************************************************************************/
	...
	public static void main(String args[]) 
    {
		 ...
		 SVDBuilderColtHowMushImputation mySVD = new SVDBuilderColtHowMushImputation(5,		 			//Classes 
												   false, 	 			//Folding	
												   false, 	 			//FT
...
/****************************************************************************************************/
	...
	for(int m = 1 ;m<=1;  m++)
	{
	  ...
	  //build the model twice, one for full and other for training set only
	 for(int trainOrFull =0;trainOrFull<=1;trainOrFull++)
	  {
		 if(trainOrFull ==0)
 ...
 */
	...
	public void buildModels(int whichModel)
	{
        ...
        int      sparsityLoopLimit  = 1;				// Control the sparse training set object
 ...
 for (int myLimit = 1;myLimit<=10;myLimit++)
 {
  	//double limit     = myLimit*1.0/10;  	
  	
	double limit  	   = 1;  	
  	double limit_user  = myLimit*1.0/10;
  	double limit_mov   = myLimit*1.0/10;
           ...
           {                   	   
	        	...
	        	myClasses = 5;
	        	 ...
	        	 if(trainingOrFullSet.equalsIgnoreCase("full")){
		            ...
		            //training
		        	datFile     = myPath   +  "sml_clusteringTrainingTrainSetStoredTF.dat";
		            testFile    = myPath +  "sml_clusteringTrainingValidationSetStoredTF.dat";
	        	   }	            
         ...
         */
          ...
          /*   if(whichModel ==13) 									
            ...
            */
            ...
            double[] data_user  = new double[numUsers+1]; 
            double[] data_mov   = new double[numMovies+1];         
            double[] data_user_Control  = new double[numUsers+1]; 
            double[] data_mov_Control   = new double[numMovies+1]; 
            
            //initialisation
            for(int i = 0; i < numMovies; i++) 
            {           	
            	data_mov[i+1] =  943 - helper.getNumberOfUsersWhoSawMovie(i+1);
            	data_mov_Control[i+1] = 0;
            }
            
            for(int j = 0; j < numUsers; j++) 
            {
               	data_user[j+1] = 1682 - helper.getNumberOfMoviesSeen(j+1);
               	data_user_Control[j+1] = 0;
            }
            ...
            for(int i = 0; i < numMovies; i++) 
            {
            	double noRatToItem = helper.getNumberOfUsersWhoSawMovie(i+1);            	 
            	
                for(int j = 0; j < numUsers; j++) 
                {
                	//uid =j
                	double noRatByUser = helper.getNumberOfMoviesSeen(j+1);
                    	...
                    	// Normal Distribution (MM)
	                    	...
	                    	modelName  = "SVDMovNormal";
		                   	...
		                   	else if(whichModel==8) 
		                   	{
		                        	...
		                        	if ( ((1682 - noRatByUser)/1682 <=limit) || ((943 - noRatToItem)/943  <=limit ))
		                        	{
		                        		//System.out.println(((myLimit *10 * data_user[j+1])/100)+ ","  +data_user_Control[j+1]);
		                        		
		                        	   if ( (data_user_Control[j+1]<((myLimit *10 * data_user[j+1])/100)) || (data_mov_Control[i+1]<((myLimit *10 * data_mov[i+1])/100)) )
		                        		{                		
		                        		   data_user_Control[j+1]++;
		                        		   data_mov_Control[i+1]++;
			                        		...
			                        		temp = ((helper.getAverageRatingForMovie(i+1)+ helper.getAverageRatingForUser(j+1) )/2.0);
			                        	}
		                        	}
			                    	...
			                    	modelName  = "SVDUserBasedCF"+(limit *10)+(limit_user*10);
			                    	//modelName  = "SVDUserBasedCF_More"+(limit *10);
		                    	}
		                   	...
		                   	else if(whichModel==9) 
		                   	{
			                    	...
			                    	data[i][j] = temp - nor/sigma;
			                    	modelName  = "SVDItemBasedCF"+(limit *10)+(limit_user*10);
			                    	//modelName  = "SVDItemBasedCF_More"+(limit *10);
		                   	...
		                   	//(User+Item CF)/2
			                    	...
			                    	data[i][j] = temp - nor/sigma;
			                    	modelName  = "SVDUserAndItemBasedCF"+(limit *10)+(limit_user*10);
			                    	//modelName  = "SVDUserAndItemBasedCF_More"+(limit *10);
		                    	}
