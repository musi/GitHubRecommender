...
class RecursiveSVD 
{
	  ...
	  //File Writers
	    FileWriter 					myWriter[], mySingleWriter;
	    
	    
  //-----------------------------------------
	/**
	 * @author Musi
	 * @param int,    FTDataWithMinRat
	 * @param int,    dataset, 0=sml, 1=ft, 2=ml
	 * @param string, "SimpleSVD", "UBCF", "IBCF"
	 * @param int,	  nor: 0=simple, 1=userNor
	 */
	public RecursiveSVD(int dum_FTDataWithMinRat, int dum_dataset, 
															String dum_CFORSimpleSVD, int dum_nor)
	{	
		...
		if(dataset==0)		//SML 
		{
		 myPath     	= "C:/Users/Musi/workspace/MusiRecommender/DataSets/SML_ML/SVD/Iterative/80/";
         trainSet		= myPath + "sml_clusteringTrainSetStoredTF.dat";
         testSet		= myPath + "sml_clusteringTestSetStoredTF.dat";         
         mainSet		= myPath + "sml_storedFeaturesRatingsTF.dat";
         FTFlag			= false;
         myClasses		= 5;
		}
		...
		else if(dataset==1)	//FT 
		{
		  ...
		  myPath     	= "C:/Users/Musi/workspace/MusiRecommender/DataSets/FT/SVD/Iterative/80/";
		...
		{
			myPath     	= "C:/Users/Musi/workspace/MusiRecommender/DataSets/ML_ML/SVD/Iterative/80/";
		 ...
		 //Files
	     myWriter = new FileWriter[20];				//see file open section
	     ...
	     optK      			= new int[7][25];   	//[imputation][iterations]
...
/****************************************************************************************************/
	...
	/*    System.out.println("Recursive SVD");
	    while(true){	    	
	    	if(2>3) break;
	    }*/
	  ...
	  int dum_FTDataWithMinRat = 1;
	  
	  //SVD object 
	  RecursiveSVD mySVD = new RecursiveSVD(dum_FTDataWithMinRat, 0, predictionMethod, dum_nor);  //0 = sml, 1= ft, 3=ml
		
	  //Open file
	  mySVD.openParameterFile();
...
/****************************************************************************************************/
	...
	public void svdBuildAndSaveIntoAFile(int modelLoop, int myLoop)
	{
        ...
        try {            
              ...
              String sparseDummyWriter = myPath + "dummySparseFile"+ modelLoop+ ".dat";            
 	          String sparseObjName = myPath + "sml_storedSparseRatingsTF_" + modelLoop + ".dat";
 	         // SVMWriter  mySVM = new SVMWriter(mainSet, trainSet, testSet, sparseDummyWriter, sparseObjName);	//main, train, test
           ...
           //destfile    = myPath + modelNormalization+ "/" + "Dim=15/" + modelName + modelNormalization + loopOfSvdWriting + ".dat";
            ...
            if(modelLoop==6 && loopOfSvdWriting ==1){
                    ...
                    if(rating == -99) 
                    {
                    	...
                    	if(loopOfSvdWriting ==1)         
                    	{
                    		if(modelLoop==1)                    			
                        		...
                        		if(tempN==-99 || tempN>myClasses  || tempN <=0){
                        							tempN = myTrainingMMh.getAverageRatingForMovie(i+1);
                    	...
                    	//Get prediction from the previously build svd model
                    			...
                    			if(flag==1) {
                    				...
                    				//We will call the function to learn the optimal dim, and that function will
                    				//retrun us the optimal no. of dimensions for all iterations for an imputation source
                    				if(optDimHasBeenFound == false) {
                    					...
                    					else { //there is no need to find optimal k in svm, put like 15
                    							//optK[modelLoop][loopOfSvdWriting-1] = 15;
                    						optK[modelLoop][1] = 15;
                    				...
                    				mySingleWriter.append(""+optK[modelLoop][loopOfSvdWriting-1]);
                    				//mySingleWriter.append(""+optK[modelLoop][1]);
                					mySingleWriter.append(",");
                					
                					if(loopOfSvdWriting ==15)
                						mySingleWriter.append("\n");
                					
                    				// loop-1, [][1] is for first iteration
                    				prepareSVDModel (previousSVDModel, optK[modelLoop][loopOfSvdWriting-1]);
                					System.out.println("Model builded @ flag="+ flag + "optK="+optK[modelLoop][loopOfSvdWriting-1] );
                					
                					/*prepareSVDModel (previousSVDModel, optK[modelLoop][1]);                					
                					System.out.println("Model builded @ flag="+ flag + "optK="+optK[modelLoop][1] );
	 ...
	 if(nor == 0)
	    ...
	    int end =0;
    	  ...
    	  {		
    	     ...
    	     for(int t=1;t<=end;t++)  //no. of iterations	 
 	     	 {   	        		 
    	    	 ...
    	    	 for(int dim = 1;dim<40;dim++)	 
    	    	 {
    	    		...
    	    		String svdFile = "";
    	    		if(dataset==1)
    	    			svdFile =  myPath + modelNormalization+ "/" + "Dim=k/" + FTDataWithMinRat + "/"+ modelName + modelNormalization + t + ".dat";
    	    		else
    	    		{
    	    		   svdFile =  myPath + modelNormalization+ "/"  + "Dim=k/" + modelName + modelNormalization + t + ".dat";
    	    	 ...
    	    	 try {		
    	    		   for(int temp=0;temp<=19;temp++)				//write a new line
    	     ...
    	     try {		
    	  	   	for(int temp=0;temp<=19;temp++)				//write new new lines for new scheme
				 	myWriter[temp].append("\n\n\n");
      		    ...
      		    public void testWithMemHelper(MemHelper testmh, int neighbours)     
    		    {
    		     ...
    		     /* System.out.println("totalExtremeErrors="+totalExtremeErrors + ", Total ="+total);
    		        ...
    		        //Get variance and Mean of MAE    		          
    		          SDMAE  	 	= rmse.getMeanErrorOfMAE();		            
    		          SDROC   		= rmse.getMeanSDErrorOfROC();
    		          SDPrecision   = rmse.getMeanSDErrorOfTopN(1);
    		          SDRecall   	= rmse.getMeanSDErrorOfTopN(2);
    		          SDF1   		= rmse.getMeanSDErrorOfTopN(3);
 ...
 public void openFiles()
	 {
		 ...
		 if(dataset==1)
			 path =myPath + "/Results/" + CFORSimpleSVD+ "/" +modelNormalization + "/" + FTDataWithMinRat + "/";
	...
	//---------------------------------------------
	 ...
	 public void openParameterFile()
	 {
		 String path =myPath + "/Results/" + CFORSimpleSVD+ "/" +"UserNor" + "/"; 
		 if(dataset==1)
			 path =myPath + "/Results/" + CFORSimpleSVD+ "/" +"UserNor" + "/" + FTDataWithMinRat + "/";
		 
		 try{
		 mySingleWriter =  new FileWriter(path +"Dim_"+modelNormalization+".csv", true);
		}
		catch( Exception E) {
	 		E.printStackTrace(); 			 
 }
	...
	//---------------------------------------------
	 ...
	 public void closeParameterFile()
	 {
	  		 
		 try{			 
				 mySingleWriter.close();
