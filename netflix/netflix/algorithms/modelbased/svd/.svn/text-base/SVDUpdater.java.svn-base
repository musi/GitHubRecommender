...
/****************************************************************************************************/
    ...
    public SVDUpdater(SingularValueDecomposition svd, int k) 
    {
        this.k = k;
        alg = new Algebra();
        ...
        //         int height,
        //         int width)
        //  Constructs and returns a new sub-range view that is a height x width sub matrix, 
        //  starting at [row,column].
        
        S = svd.getS().viewPart(0, 0, k, k).copy();
        U = svd.getU().viewPart(0, 0, svd.getU().rows(), k ).copy(); //rows
...
/****************************************************************************************************/
    ...
    private void rank1Modification(DoubleMatrix1D a, DoubleMatrix1D b) 
    {
        ...
        //pVec = b - Vn
        DoubleMatrix1D qVec = MatrixHelper.subVectors(b, alg.mult(alg.transpose(Vt), n));
        ...
        DoubleMatrix2D newU = alg.mult(MatrixHelper.appendToMatrix(U,P), newSVD.getU());
...
/****************************************************************************************************/
    ...
    public void addUser(DoubleMatrix1D user) 
    {
        ...
        DoubleMatrix2D matrix = new DenseDoubleMatrix2D(S.rows() + 1, S.columns() + 1);
        ...
        SingularValueDecomposition newSVD = new SingularValueDecomposition(matrix);
