...
import netflix.algorithms.memorybased.ftmemreader.FTNDepthData;
...
/************************************************************************************************************************/
public class FTNDepthFilterAndWeight

{
 ...
 /************************************************************************************************************************/
    ...
    public FTNDepthFilterAndWeight(FTMemHelper mh) 
  ...
  /************************************************************************************************************************/
    ...
    public FTNDepthFilterAndWeight(FTMemHelper mh, int options) 
 ...
 /************************************************************************************************************************/
  ...
  public double recommendSKU(int activeUser, int targetMovie, 
		  					int depth, IntArrayList setOfUsers, 
	  						 FTMemHelper testMMh,  
	  						 IntArrayList givenMovies, int currentK)
	  					  
   {
	       ...
	       //Filter movies which have less than two ratings
	       int limit = users.size();	       
	       ...
	       for (int i = 0; i < limit; i++)       
	       {
	    	    ...
	    	    if(uid==0) System.out.println("uid =0");
	    	    if (setOfUsers.contains(uid)) //this user is my friend 	    	    	
	    	    {
	    	    	currentWeight  = weight(activeUser, uid);
	    	    	uidToWeight.put(uid, currentWeight);
	    	    	howMuchUsersRatedThis++;   
	    	    }
	      }
	       
	       if( howMuchUsersRatedThis ==0)   return -10;
	       ...
	       if (currentK < limit) limit = currentK; 	//by default all              
...
/************************************************************************************************************************/
  ...
  public double recommendSKR(  int activeUser, int targetMovie, 
		  					   int currentK,
		  					   IntArrayList setOfUsers
		  					)
   ...
   {       double currWeight, weightSum = 0, voteSum = 0, weightSumAbs = 0;
		...
		*/   
	      ...
	      // givenVotesForAUser = mh.getMoviesSeenByUser(activeUser);		//These movies should be there for active usr to fnd similariy etc
	      // for ()	       
	       ...
	       for (int i = 0; i < limit; i++)       
	       {
	    	    ...
	    	    if(uid==0) {System.out.println("uid =0"); System.exit(1);}
 ...
 /************************************************************************************************************************/
  ...
  public double recommendSLOO(  int activeUser, int targetMovie, 
		  						int depth, IntArrayList setOfUsers, 
		  						IntArrayList givenMovies, int currentK, double activeUserAverage
		  					 )	    
   {
	...
	//These are the observale votes
        ...
        //Now Find all the users who are my friends in a depth (Explicit network)
  //    IntArrayList users = NData.getNeighboursInDepth(activeUser, depth);
        LongArrayList users = mh.getUsersWhoSawMovie(targetMovie);		//this should also be done in (total set - test set)
        int friendLimit = setOfUsers.size();
        int userLimit   = users.size(); 
       
        //Filter movies which have less than two ratings
        if(userLimit<=1) return -10;
        //_____________________________________________________
        //go through all users who saw the target movie, to find weights
        ...
        for (int i = 0; i < userLimit; i++)
        {
        	...
        	if (uid != activeUser && setOfUsers.contains(uid)) //this user is my friend and not I
        	{
        		currentWeight  = weight(activeUser, uid);
        		uidToWeight.put(uid, currentWeight);
        		howMuchUsersRatedThis++;
        ...
        //tempWeight = quickSort.quicksort(tempWeight);
      //topWeights = quickSort.quicksort(topWeights);
        myUsers = uidToWeight.keys();
        myWeights = uidToWeight.values();
        uidToWeight.pairsSortedByValue(myUsers, myWeights);
        
       if (currentK < userLimit) userLimit = currentK; 	//by default all              
       int totalNeighbourFound =myUsers.size();       
     
        
        for (int i = totalNeighbourFound-1, myTotal=0; i >=0; i--, myTotal++)       
        {
        	if(myTotal==userLimit) break;        	
        	uid = myUsers.get(i);        	
            if (uid != activeUser)
            {
            	currentWeight= myWeights.get(i);
        ...
        if (voteSum<0)    {System.out.println(", weight sum=" + voteSum);
     ...
     //   else answer = activeUserAverage;
        ...
        double answer= activeUserAverage + voteSum;
      //System.out.println ("prediction ="+ answer);
        ...
        /*	  System.out.println(" Errror");
              ...
              System.out.print(", total weighted users= " + totalNeighbourFound);
